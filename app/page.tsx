      ...
      <section className="relative min-h-screen border-b border-white/[0.08] pt-20">
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-80px)] max-w-[1400px] items-center gap-10 px-6 py-16 lg:grid-cols-[1.05fr_.95fr] lg:px-10 lg:py-20">
          <div className="relative z-10">
            ...
            <h1 className="font-display max-w-5xl text-[clamp(4rem,9vw,9.5rem)] font-semibold leading-[0.84]">
              Transform<br />fragmented<br /><span className="text-white/30">ideas.</span>
            </h1>
            <div className="mt-10 flex max-w-xl flex-col gap-6 sm:flex-row sm:items-end">
              ...
              <a href="#work" className="group flex shrink-0 items-center gap-3 text-sm font-medium">Explore our work <span className="text-[#ff5a1f] transition group-hover:translate-x-1"><Arrow /></span></a>
            </div>
          </div>
          <HeroOrbit />
        </div>
      ...
      <section className="border-b border-white/[0.08]" aria-label="Company statistics">
        <div className="mx-auto grid max-w-[1400px] grid-cols-2 divide-white/[0.08] px-6 sm:grid-cols-4 sm:divide-x lg:px-10">
          {[["50+", "Projects delivered"], ["30+", "Digital experiences"], ["5+", "Years of craft"], ["10+", "Industries explored"]].map(([value, label]) => (
            <div key={label} className="px-5 py-10 first:pl-0 last:pr-0 sm:px-8"><p className="text-4xl font-semibold tracking-[-0.05em]">{value}</p><p className="mt-2 text-xs uppercase tracking-[0.16em] text-white/35">{label}</p></div>
          ))}
        </div>
      </section>
      ...
      <section id="solutions" className="mx-auto max-w-[1400px] px-6 py-28 lg:px-10 lg:py-40">
        <div className="grid gap-16 lg:grid-cols-[.8fr_1.2fr]">
          ...
          <div className="border-t border-white/10">
            {solutions.map(([number, title, description]) => <div key={number} className="grid gap-6 border-b border-white/10 py-9 sm:grid-cols-[80px_1fr_1fr]"><span className="text-xs text-white/30">{number}</span><h3 className="text-2xl font-medium">{title}</h3><p className="max-w-md text-sm leading-6 text-white/45">{description}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <section id="work" className="border-y border-white/[0.08] bg-[#0b0b0b]">
        <div className="mx-auto max-w-[1400px] px-6 py-28 lg:px-10 lg:py-40">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {projects.map((project, index) => <article key={project.title} className={`${index === 0 ? "md:col-span-2" : ""} group relative min-h-[360px] overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#111] p-7`}><div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(255,90,31,.18),transparent_40%)] transition duration-700 group-hover:scale-110" /><div className="absolute right-[10%] top-[16%] h-44 w-44 rotate-12 rounded-[30%] border border-[#ff5a1f]/20 bg-[#ff5a1f]/10 shadow-[0_0_90px_rgba(255,90,31,.18)] transition duration-700 group-hover:rotate-0 group-hover:scale-110" /><div className="absolute bottom-7 left-7 right-7 flex items-end justify-between"><div><p className="mb-2 text-xs uppercase tracking-[0.18em] text-[#ff6a2a]">{project.type}</p><h3 className="text-2xl font-medium">{project.title}</h3></div><span className="text-sm text-white/30">{project.year} <Arrow /></span></div></article>)}
          </div>
        </div>
      </section>
