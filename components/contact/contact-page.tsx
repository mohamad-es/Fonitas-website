        ...
        <div className="relative mx-auto grid max-w-[1400px] gap-16 px-6 lg:grid-cols-[1.05fr_.75fr] lg:items-end lg:px-10">
          <div>
            ...
            <h1 className="reveal-up reveal-delay-1 font-display mt-7 max-w-5xl text-5xl font-semibold leading-[.94] sm:text-7xl lg:text-[8rem]">
              Bring your
              <br />
              <span className="text-white/35">application</span> to market.
            </h1>

            <p className="reveal-up reveal-delay-2 mt-8 max-w-2xl text-base leading-7 text-white/45 sm:text-lg">
              Tell us what you are building, where you want to publish it,
              and where you need support. We can start with publishing and
              expand into the wider operating workflow.
            </p>
            ...
            <div className="relative border border-white/10 bg-white/[0.025] p-6 sm:p-8">
              ...
              <div className="py-10">
                <div className="flex items-center gap-4">
                  <div className="flex h-16 w-16 items-center justify-center border border-white/10 bg-white/[0.035] text-xs text-white/60">
                    YOU
                  </div>
                  <div className="h-px flex-1 bg-gradient-to-r from-white/15 via-[#ff5a1f]/70 to-white/10" />
                  <div className="relative flex h-20 w-20 items-center justify-center rounded-full border border-[#ff5a1f]/40 bg-[#ff5a1f]/10 text-[11px] font-semibold tracking-[0.16em] text-[#ff7a3d] shadow-[0_0_50px_rgba(255,90,31,.15)]">
                    FONITAS
                  </div>
                </div>

                <div className="mt-10 border-l border-[#ff5a1f]/30 pl-5">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-[#ff7a3d]">
                    First signal
                  </p>
                  <p className="mt-3 text-sm leading-6 text-white/60">
                    Application → QA → Publishing → Settlement
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 border-t border-white/[0.08]">
                {["Application", "Target stores", "Markets", "Revenue model"].map((item, index) => (
                  <div
                    key={item}
                    className={`border-white/[0.08] px-3 py-3 text-[10px] uppercase tracking-[0.14em] text-white/30 ${
                      index % 2 === 0 ? "border-r" : ""
                    } ${index < 2 ? "border-b" : ""}`}
                  >
                    {item}
        ...
        <div className="grid gap-16 lg:grid-cols-[.55fr_1fr]">
          <div>
            ...
            <p className="mt-6 max-w-md text-sm leading-7 text-white/40">
              Give us enough context to understand the release, the operating
              requirements, and the path ahead.
            </p>
          </div>
          ...
          <div className="border-t border-white/[0.08]">
            {routes.map(([number, title, text]) => (
              <div
                key={number}
                className="group grid gap-5 border-b border-white/[0.08] py-7 sm:grid-cols-[60px_1fr_auto] sm:items-center"
              >
                <span className="text-xs text-[#ff5a1f]">{number}</span>
                <div>
                  <h3 className="text-base font-medium text-white/80 transition group-hover:text-white">
                    {title}
                  </h3>
                  <p className="mt-1 text-sm leading-6 text-white/35">{text}</p>
                </div>
                <span className="text-lg text-white/20 transition group-hover:translate-x-1 group-hover:text-[#ff7a3d]">
                  ↗
                </span>
              </div>
            ))}
      ...
      <section className="border-y border-white/[0.07] bg-white/[0.012]">
        <div className="mx-auto grid max-w-[1400px] gap-14 px-6 py-24 lg:grid-cols-[.62fr_1fr] lg:px-10 lg:py-32">
          <div>
            ...
            <p className="mt-6 max-w-md text-sm leading-7 text-white/40">
              These are the signals that help map your application to the
              publishing workflow.
            </p>

            <div className="mt-10 space-y-0 border-t border-white/[0.08]">
              {briefItems.map(([label, detail], index) => (
            ...
            <div className="mt-10 border-t border-white/[0.08] pt-5">
              ...
              <a
                href="mailto:hello@fonitas.com"
                className="mt-2 inline-block text-sm text-white/65 transition hover:text-[#ff7a3d]"
              >
                hello@fonitas.com
              </a>
            </div>
          ...
          <div className="border border-white/10 bg-[#0b0b0b] p-6 sm:p-8 lg:p-10">
            {submitted ? (
              <div className="flex min-h-[520px] flex-col items-center justify-center text-center">
                <span className="flex h-16 w-16 items-center justify-center rounded-full border border-[#ff5a1f]/30 bg-[#ff5a1f]/10 text-xl text-[#ff7a3d]">
                  ✓
                </span>
                <p className="mt-6 text-[10px] uppercase tracking-[0.24em] text-[#ff7a3d]">
                  Signal received
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <label className="block text-sm text-white/65">
                    ...
                    <input
                      name="name"
                      required
                      className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3.5 text-sm text-white outline-none transition placeholder:text-white/20 focus:border-[#ff5a1f]/60"
                ...
                <label className="block text-sm text-white/65">
                  ...
                  <select
                    ...
                    required
                    defaultValue=""
                    className="mt-2 w-full rounded-2xl border border-white/10 bg-[#101010] px-4 py-3.5 text-sm text-white outline-none transition focus:border-[#ff5a1f]/60"
                  >
                ...
                <label className="block text-sm text-white/65">
                  ...
                  <textarea
                    ...
                    rows={7}
                    required
                    className="mt-2 w-full resize-none rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3.5 text-sm leading-6 text-white outline-none transition placeholder:text-white/20 focus:border-[#ff5a1f]/60"
                    placeholder="What are you building, where do you want to publish it, and what support do you need?"
                  />
                </label>

                <button
                  type="submit"
                  className="w-full rounded-full bg-[#ff5a1f] px-5 py-4 text-sm font-semibold text-black transition hover:bg-[#ff7a3d]"
                >
                  Send request ↗
                </button>
              </form>
