...
function OpportunityCard({ opportunity, t }: { opportunity: (typeof investmentOpportunities)[number]; t: ReturnType<typeof useTranslations> }) {
  return (
    <Link href={`/investors/${opportunity.slug}`} className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010] transition duration-500 hover:-translate-y-1 hover:border-[#ff5a1f]/35">
      ...
      <div className="p-7 sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div><h2 className="font-display text-3xl font-semibold">{opportunity.name}</h2><p className="mt-2 text-sm text-white/45">{t(`${opportunity.name} tagline`)}</p></div>
          <span className="text-[#ff5a1f]"><Arrow /></span>
        </div>
        <div className="mt-6 grid grid-cols-2 gap-3">
          {opportunity.metrics.map((metric) => <div key={metric.label} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4"><p className="text-[8px] uppercase tracking-[0.18em] text-white/20">{t(metric.label)}</p><p className="mt-2 text-sm font-medium text-white/75">{metric.value}</p></div>)}
        </div>
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#070707] text-white">
            ...
            <div className="relative min-h-[360px] overflow-hidden rounded-[32px] border border-[#ff5a1f]/20 bg-[#0d0d0d] p-6 shadow-[0_0_90px_rgba(255,90,31,0.07)]">
              ...
              <div className="relative flex h-full min-h-[308px] flex-col justify-between">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full border border-[#ff5a1f]/20 bg-[#ff5a1f]/10 px-3 py-1 text-[8px] uppercase tracking-[0.18em] text-[#ff6a2a]">{t("Investment opportunities")}</span>
                </div>
                <div className="relative flex flex-1 items-center justify-center py-5">
                  ...
                  <div className="relative z-10 flex items-center gap-2 sm:gap-4">
                    ...
                    <div className="flex w-[100px] flex-col items-center text-center sm:w-[125px]">
                      ...
                      <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-white">{t("Investment")}</p>
      ...
      <section className="mx-auto max-w-[1400px] px-6 py-20 lg:px-10 lg:py-28">
        <div className="mb-10 flex items-end justify-between gap-6"><div><p className="text-[10px] uppercase tracking-[0.25em] text-[#ff5a1f]">{t("Opportunities")}</p><h2 className="font-display mt-3 text-4xl font-semibold sm:text-5xl">{t("Selected applications")}</h2></div><span className="hidden text-[9px] uppercase tracking-[0.2em] text-white/20 sm:block">{investmentOpportunities.length.toString().padStart(2,"0")} opportunities</span></div>
