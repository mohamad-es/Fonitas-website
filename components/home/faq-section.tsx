      ...
      <div className="mx-auto max-w-[1400px] px-6 py-20 sm:py-28 lg:px-10 lg:py-40">
        <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-16">
          <div className="lg:sticky lg:top-32 lg:self-start">
            ...
            <h2 className="font-display max-w-xl text-4xl font-semibold leading-[.92] sm:text-6xl lg:text-7xl">{t("The important parts,")} <span className="text-white/30">{t("made clear.")}</span></h2>
            <p className="mt-7 max-w-md text-sm leading-6 text-white/35 sm:mt-8">{t("A concise view of the publishing model, source-code boundary, store ownership and financial lifecycle.")}</p>
          </div>
          <div className="border-t border-white/10">
            {faqs.map(([question, answer], index) => (
              <details key={t(question)} className="group border-b border-white/10">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-5 py-6 text-base font-medium text-white/80 transition hover:text-white sm:gap-8 sm:py-7 sm:text-xl">
                  <span className="flex min-w-0 items-start gap-4"><span className="mt-1 shrink-0 text-[10px] text-[#ff5a1f]">{String(index + 1).padStart(2, "0")}</span><span>{t(question)}</span></span>
                ...
                <div className="max-w-3xl pb-7 pl-0 text-sm leading-7 text-white/40 sm:pb-8 sm:pl-12">{t(answer)}</div>
              </details>
            ))}
          </div>
        </div>
