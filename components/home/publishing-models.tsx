      ...
      <div className="mx-auto max-w-[1400px] px-6 py-20 sm:py-28 lg:px-10 lg:py-40">
        <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
          <div className="lg:sticky lg:top-32 lg:self-start">
            ...
            <h2 className="font-display max-w-xl text-4xl font-semibold leading-[.92] sm:text-6xl lg:text-7xl">One publishing core. <span className="text-white/30">Different operating models.</span></h2>
            <p className="mt-7 max-w-md text-sm leading-6 text-white/35 sm:mt-8">The architecture supports different levels of involvement without creating a separate publishing architecture for each model.</p>
          </div>
              ...
              {models.map((model, index) => (
                <article key={model.number} className="group relative border-t border-white/10 py-7 first:border-t-0 sm:py-10 lg:pl-16">
                  ...
                  <div className="grid gap-6 lg:grid-cols-[1fr_1.05fr] lg:items-center lg:gap-7">
                    <div>
                      <div className="flex flex-wrap items-center gap-3"><span className="text-xs text-[#ff5a1f]">{model.number}</span><span className="text-[9px] uppercase tracking-[.22em] text-white/25">{model.tag}</span></div>
                      <h3 className="mt-3 font-display text-2xl font-semibold tracking-[-.04em] sm:text-4xl">{model.title}</h3>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {model.flow.map((step, stepIndex) => (
                          <div key={step} className="flex items-center gap-2">
                            <span className="max-w-full rounded-full border border-white/10 bg-white/[.02] px-3 py-2 text-[10px] leading-4 text-white/50">{step}</span>
