      ...
      <div className="mx-auto max-w-[1400px] px-6 py-28 lg:px-10 lg:py-40">
        <div className="grid gap-14 lg:grid-cols-[.7fr_1.3fr] lg:gap-20">
          <div className="lg:sticky lg:top-32 lg:self-start">
            <p className="mb-5 text-xs uppercase tracking-[.25em] text-[#ff5a1f]">{t("Why Fonitas")}</p>
            <h2 className="font-display max-w-xl text-5xl font-semibold leading-[.9] sm:text-7xl">{t("Built around")} <span className="text-white/30">{t("accountability.")}</span></h2>
            <p className="mt-8 max-w-md text-sm leading-7 text-white/35">A controlled publishing operation needs more than a workflow. It needs visible responsibility from release preparation through financial close.</p>
          </div>
              ...
              {principles.map(([number, title, description], index) => (
                <article key={number} className="group relative border-t border-white/10 py-7 first:border-t-0 sm:py-9">
                  <div className="grid gap-5 sm:grid-cols-[72px_190px_1fr] sm:items-start">
                    ...
                    <h3 className="font-display text-3xl font-semibold transition group-hover:text-[#ff6a2a]">{t(title)}</h3>
                    <p className="max-w-lg text-sm leading-6 text-white/40">{t(description)}</p>
                  </div>
