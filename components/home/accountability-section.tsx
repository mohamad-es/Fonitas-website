              ...
              {principles.map(([number, title, description], index) => (
                <article key={number} className="group relative border-t border-white/10 py-7 first:border-t-0 sm:py-9">
                  <div className="grid gap-5 sm:grid-cols-[72px_190px_1fr] sm:items-start">
                    ...
                    <h3 className="font-display text-3xl font-semibold transition group-hover:text-[#ff6a2a]">{title}</h3>
                    <p className="max-w-lg text-sm leading-6 text-white/40">{description}</p>
                  </div>
