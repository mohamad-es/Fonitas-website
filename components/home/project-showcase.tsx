        ...
        <div className="mt-6 grid grid-cols-3 gap-2">
          {["Platform", "Markets", "Revenue"].map((x) => (
            <div key={x} className="rounded-xl border border-white/10 p-3">
              <span className="text-[7px] uppercase tracking-[.15em] text-white/20">{t(x)}</span>
      ...
      <div className="absolute left-7 right-7 top-20">
        {checks.map((check, index) => (
          <div key={check} className="mb-3 flex items-center gap-3">
            ...
            <span className="text-[9px] uppercase tracking-[.15em] text-white/45">{t(check)}</span>
    ...
    <div className="relative min-h-[300px] overflow-hidden bg-[#0d0d0d] p-7">
      ...
      <div className="absolute bottom-7 left-7 right-7 flex flex-wrap justify-center gap-2">
        {["Apple App Store", "Google Play", "Microsoft Store"].map((store, index) => (
          <span key={store} className={`rounded-full border px-3 py-2 text-[7px] uppercase tracking-[.1em] ${index === 0 ? "border-[#ff5a1f]/30 text-[#ff6a2a]" : "border-white/10 text-white/25"}`}>{t(store)}</span>
        ))}
      </div>
    </div>
  );
        ...
        <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
          ...
          <div className="p-7 sm:p-9 lg:p-10">
            <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]">
              <span className="text-[#ff6a2a]">01 / {t("Application")}</span>
        ...
        <div className="grid gap-5">
          <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
            ...
            <div className="p-7 sm:p-8">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]"><span className="text-[#ff6a2a]">02 / QA & Compliance</span><span className="text-white/25">{t("Validate")}</span></div>
              <h3 className="font-display mt-3 text-2xl font-semibold">{t("Validate the release")}</h3>
          ...
          <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
            <PublishingVisual />
            <div className="p-7 sm:p-8">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]"><span className="text-[#ff6a2a]">03 / Publishing</span><span className="text-white/25">{t("Submit")}</span></div>
              <h3 className="font-display mt-3 text-2xl font-semibold">{t("Submit to the store")}</h3>
