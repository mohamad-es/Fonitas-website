...
const stages = [
  {
    number: "01",
    type: "Application",
    title: "{t("Register")} the application",
  ...
  },
  {
    number: "02",
    type: "QA & Compliance",
    title: "{t("{t("Validate")} the release")}",
    ...
    <div className="relative h-full min-h-[360px] overflow-hidden bg-[#0f0f0f] p-7">
      ...
      <div className="absolute left-[11%] top-[15%] h-[62%] w-[72%] rotate-[-5deg] rounded-[26px] border border-white/10 bg-[#151515] p-5 shadow-2xl transition duration-700 group-hover:rotate-[-2deg] group-hover:scale-[1.02]">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <span className="text-[9px] uppercase tracking-[.2em] text-white/25">{t("Application record")}</span>
          <span className="rounded-full bg-[#ff5a1f]/10 px-2 py-1 text-[8px] text-[#ff6a2a]">{t("REGISTERED")}</span>
        </div>
        <div className="mt-6 grid grid-cols-3 gap-2">
          {["{t("Platform")}", "{t("Markets")}", "{t("Revenue")}"].map((x) => (
      ...
      <span className="absolute bottom-[9%] right-[9%] rounded-full border border-white/10 bg-[#0b0b0b]/90 px-4 py-2 text-[8px] uppercase tracking-[.18em] text-white/40">{t("01 / Intake")}</span>
    </div>
  );
}

function QAVisual() {
  const checks = ["SHA-256", "{t("Security scan")}", "{t("Manifest")}", "{t("Permissions")}", "{t("Manual QA")}"];
  return (
    <div className="relative min-h-[300px] overflow-hidden bg-[#111] p-7">
      ...
      <div className="absolute left-7 right-7 top-7 flex items-center justify-between text-[8px] uppercase tracking-[.18em] text-white/25">
        <span>{t("Release validation")}</span><span>{t("Ready gate")}</span>
      </div>
      ...
      <div className="absolute bottom-6 left-7 right-7 flex items-center justify-between border-t border-white/10 pt-4">
        <span className="text-[8px] uppercase tracking-[.18em] text-white/25">{t("QA status")}</span>
        <span className="text-[10px] font-medium text-[#ff6a2a]">{t("READY FOR SUBMISSION")}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative min-h-[300px] overflow-hidden bg-[#0d0d0d] p-7">
      ...
      <div className="absolute bottom-7 left-7 right-7 flex flex-wrap justify-center gap-2">
        {["{t("Apple App Store")}", "{t("Google Play")}", "{t("Microsoft Store")}"].map((store, index) => (
  ...
  return (
    <div className="relative">
        ...
        <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
          ...
          <div className="p-7 sm:p-9 lg:p-10">
            <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]">
              <span className="text-[#ff6a2a]">{t("01 / Application")}</span>
              <span className="text-white/25">{t("Register")}</span>
            </div>
            <h3 className="font-display mt-4 text-4xl font-semibold leading-[.95] sm:text-5xl">{t(stages[0].title)}</h3>
            <p className="mt-5 max-w-xl text-sm leading-6 text-white/40">{t(stages[0].description)}</p>
          </div>
        </article>
        ...
        <div className="grid gap-5">
          <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
            ...
            <div className="p-7 sm:p-8">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]"><span className="text-[#ff6a2a]">{t("02 / QA & Compliance")}</span><span className="text-white/25">{t("Validate")}</span></div>
              <h3 className="font-display mt-3 text-2xl font-semibold">{t("{t("Validate")} the release")}</h3>
              <p className="mt-3 text-sm leading-6 text-white/40">{t(stages[1].description)}</p>
            </div>
          </article>
          ...
          <article className="group overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
            <PublishingVisual />
            <div className="p-7 sm:p-8">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.22em]"><span className="text-[#ff6a2a]">{t("03 / Publishing")}</span><span className="text-white/25">{t("Submit")}</span></div>
              <h3 className="font-display mt-3 text-2xl font-semibold">{t("{t("Submit")} to the store")}</h3>
              <p className="mt-3 text-sm leading-6 text-white/40">{t(stages[2].description)}</p>
            </div>
          </article>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-3 text-[9px] uppercase tracking-[.18em] text-white/20">
        <span>{t("Release path")}</span><span className="text-[#ff5a1f]/60">Application</span><span><ArrowRight aria-hidden="true" className="inline h-3.5 w-3.5" strokeWidth={1.8} /></span><span className="text-[#ff5a1f]/60">{t("Validate")}</span><span><ArrowRight aria-hidden="true" className="inline h-3.5 w-3.5" strokeWidth={1.8} /></span><span className="text-[#ff5a1f]/60">Publish</span>
      </div>
    </div>
  );
}