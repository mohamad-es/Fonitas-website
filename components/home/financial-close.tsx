...
import { use{t("State")} } from "react";
import { useTranslations } from "next-intl";

const stages = [
  ["01", "{t("Revenue")}", "{t("Revenue")} facts enter the financial workflow after publication."],
  ["02", "{t("Reconciliation")}", "External revenue is matched against costs and contractual rules."],
  ["03", "{t("Owner share")}", "{t("The applicable contractual share becomes a financial fact.")}"],
  ["04", "{t("Settlement")}", "{t("Settlement")} records close the financial obligation."],
  ["05", "{t("State")}ment", "{t("The resulting statement keeps the financial history explicit.")}"],
  ["06", "{t("Payment")}", "{t("Payment")} completes the operational financial loop."],
];

export function FinancialClose() {
  const t = useTranslations();
  const [active, setActive] = use{t("State")}(0);
          ...
          <div className="relative overflow-hidden rounded-[24px] border border-white/10 bg-[#0d0d0d] shadow-[0_30px_100px_rgba(0,0,0,.3)] sm:rounded-[32px]">
            ...
            <div className="relative">
              <div className="flex items-start justify-between gap-4 border-b border-white/10 px-5 py-5 sm:px-8">
                <div><p className="text-[9px] uppercase tracking-[.22em] text-white/25">{t("Finance / Application")}</p><p className="mt-1 text-sm font-medium">{t("Revenue")} operations</p></div>
                <span className="shrink-0 rounded-full border border-[#ff5a1f]/25 bg-[#ff5a1f]/[.05] px-3 py-1.5 text-[8px] uppercase tracking-[.16em] text-[#ff6a2a]">{t("Recorded")}</span>
              </div>
              <div className="grid lg:grid-cols-[190px_1fr]">
                ...
                <div className="relative min-h-[360px] p-6 sm:min-h-[430px] sm:p-10">
                  ...
                  <div className="relative flex h-full flex-col">
                    <div className="flex items-center justify-between gap-4 text-[9px] uppercase tracking-[.2em]"><span className="text-[#ff6a2a]">{stage[0]} / {t(stage[1])}</span><span className="text-white/20">{t("Financial fact")}</span></div>
                    <div className="mt-10 flex flex-1 flex-col justify-center sm:mt-12">
                      ...
                      <h3 className="font-display mt-6 text-4xl font-semibold leading-[.9] sm:mt-8 sm:text-6xl">{t(stage[1])}</h3>
                      <p className="mt-4 max-w-xl text-sm leading-6 text-white/40 sm:mt-5 sm:leading-7 sm:text-base">{t(stage[2])}</p>
                    </div>
