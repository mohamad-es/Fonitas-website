  ...
  return (
    <section id="pricing" className="border-y border-white/[0.08] bg-[#090909]">
      <div className="mx-auto max-w-[1400px] px-6 py-28 lg:px-10 lg:py-40">
              ...
              {plans.map((plan, index) => (
                <article key={plan.number} className={`group relative overflow-hidden rounded-[26px] border p-6 transition duration-500 sm:p-8 ${index===1?"border-[#ff5a1f]/35 bg-[#ff5a1f]/[.045]":"border-white/10 bg-[#101010] hover:border-white/20"}`}>
                  <div className="relative grid gap-7 lg:grid-cols-[56px_1fr_auto] lg:items-start">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-3"><span className="text-[9px] uppercase tracking-[.22em] text-white/25">{index===0?"CORE":index===1?"EXPANDED":"LONG-TERM"}</span>{index===1&&<span className="rounded-full border border-[#ff5a1f]/25 px-2 py-1 text-[8px] uppercase tracking-[.16em] text-[#ff6a2a]">Expanded</span>}</div>
                      <h3 className="font-display mt-3 text-3xl font-semibold">{plan.title}</h3>
        ...
        <div className="mt-10 border-t border-white/10 pt-6 text-[10px] uppercase tracking-[.16em] text-white/20">Scope and commercial terms depend on the application, target stores, markets, publishing responsibilities and agreed commercial model.</div>
      </div>
    </section>
  );
}