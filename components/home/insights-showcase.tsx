    ...
    return (
      <div className="relative h-full min-h-[420px] overflow-hidden bg-[#0d0d0d] p-7 lg:min-h-[500px]">
        ...
        {[["PRODUCTS","left-[9%] top-[18%]"],["PRICING","right-[9%] top-[22%]"],["ENTITLEMENTS","right-[12%] bottom-[19%]"],["USAGE","left-[12%] bottom-[18%]"]].map(([label,pos]) => (
          <div key={label} className={`absolute ${pos} rounded-full border border-white/10 bg-[#111]/90 px-3 py-2 text-[8px] uppercase tracking-[.15em] text-white/40 backdrop-blur-xl`}>{label}</div>
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="relative h-full min-h-[230px] overflow-hidden bg-[#0d0d0d] p-6">
      ...
      {[["AUTH","left-[9%] top-[20%]"],["STORAGE","right-[8%] top-[23%]"],["MESSAGING","left-[11%] bottom-[18%]"],["ANALYTICS","right-[8%] bottom-[18%]"]].map(([label,pos])=>(
        <div key={label} className={`absolute ${pos} flex items-center gap-2 rounded-full border border-white/10 bg-[#111]/90 px-3 py-2 text-[8px] uppercase tracking-[.14em] text-white/40 backdrop-blur-xl`}><span className="h-1.5 w-1.5 rounded-full bg-[#ff5a1f]" />{label}</div>
      ))}
    </div>
  );
}
      ...
      <article className="group relative overflow-hidden rounded-[30px] border border-white/10 bg-[#101010]">
        ...
        <div className="relative p-7 sm:p-9 lg:p-11">
          <div className="max-w-2xl rounded-[24px] border border-white/10 bg-[#0b0b0b]/90 p-7 backdrop-blur-xl sm:p-9">
            ...
            <p className="mt-6 max-w-xl text-sm leading-6 text-white/40">{roadmap[0].description}</p>
            <div className="mt-8 flex items-center gap-3 text-[9px] uppercase tracking-[.2em] text-[#ff6a2a]"><span className="h-px w-8 bg-[#ff5a1f]/50" /> Long-term platform direction</div>
          </div>
        </div>
      </article>
        ...
        {roadmap.slice(1).map((item) => (
          <article key={item.number} className="group grid overflow-hidden rounded-[30px] border border-white/10 bg-[#101010] sm:grid-cols-[.9fr_1.1fr] lg:grid-cols-1">
            ...
            <div className="flex flex-col justify-between p-7 sm:p-8 lg:min-h-[220px]">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.2em] text-white/30"><span>{item.category}</span><span>{item.number} / FUTURE</span></div>
              <div className="mt-8"><h3 className="font-display text-3xl font-semibold leading-tight">{item.title}</h3><p className="mt-4 text-sm leading-6 text-white/40">{item.description}</p></div>
            </div>
          </article>
        ))}
      </div>
