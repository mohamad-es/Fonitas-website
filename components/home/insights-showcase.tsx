...
const roadmap = [
  { number: "01", key: "commercial", visual: "commercial" },
  { number: "02", key: "growth", visual: "growth" },
  { number: "03", key: "platform", visual: "platform" },
];

function RoadmapVisual({ type, t }: { type: string; t: ReturnType<typeof useTranslations> }) {
  if (type === "commercial") {
    const labels = [
      ["products", "left-[9%] top-[18%]"],
      ["pricing", "right-[9%] top-[22%]"],
      ["entitlements", "right-[12%] bottom-[19%]"],
      ["usage", "left-[12%] bottom-[18%]"],
    ] as const;
    return (
      <div className="relative min-h-[420px] overflow-hidden bg-[#0d0d0d] p-7 lg:h-[500px]">
        ...
        {labels.map(([key, pos]) => (
          <div key={key} className={`absolute ${pos} rounded-full border border-white/10 bg-[#111]/90 px-3 py-2 text-[8px] uppercase tracking-[.15em] text-white/40 backdrop-blur-xl`}>{t(`insights.visual.commercial.${key}`)}</div>
        ))}
      </div>
    );
  }
  ...
  if (type === "growth") {
    return (
      <div className="relative min-h-[230px] overflow-hidden bg-[#101010] p-6">
        ...
        <div className="absolute left-7 top-7 rounded-full border border-white/10 bg-[#0b0b0b]/90 px-3 py-2 text-[8px] uppercase tracking-[.18em] text-[#ff6a2a]">{t("insights.visual.growth.promotionSignals")}</div>
        <div className="absolute bottom-7 right-7 rounded-full border border-white/10 bg-[#0b0b0b]/90 px-3 py-2 text-[8px] uppercase tracking-[.18em] text-white/35">{t("insights.visual.growth.future")}</div>
      </div>
    );
  }
  ...
  const labels = [
    ["auth", "left-[9%] top-[20%]"],
    ["storage", "right-[8%] top-[23%]"],
    ["messaging", "left-[11%] bottom-[18%]"],
    ["analytics", "right-[8%] bottom-[18%]"],
  ] as const;
  return (
    <div className="relative min-h-[230px] overflow-hidden bg-[#0d0d0d] p-6">
      ...
      {labels.map(([key,pos])=>(
        <div key={key} className={`absolute ${pos} flex items-center gap-2 rounded-full border border-white/10 bg-[#111]/90 px-3 py-2 text-[8px] uppercase tracking-[.14em] text-white/40 backdrop-blur-xl`}><span className="h-1.5 w-1.5 rounded-full bg-[#ff5a1f]" />{t(`insights.visual.platform.${key}`)}</div>
      ))}
    </div>
  );
}
        ...
        {roadmap.slice(1).map((item) => (
          <article key={item.number} className="group grid overflow-hidden rounded-[30px] border border-white/10 bg-[#101010] sm:grid-cols-[.9fr_1.1fr] lg:grid-cols-1">
            ...
            <div className="flex flex-col justify-between p-7 sm:p-8 lg:min-h-[220px]">
              <div className="flex items-center justify-between text-[9px] uppercase tracking-[.2em] text-white/30"><span>{t(`insights.roadmap.${item.key}.category`)}</span><span>{item.number} / {t("insights.future")}</span></div>
              <div className="mt-8"><h3 className="font-display text-3xl font-semibold leading-tight">{t(`insights.roadmap.${item.key}.title`)}</h3><p className="mt-4 text-sm leading-6 text-white/40">{t(`insights.roadmap.${item.key}.description`)}</p></div>
            </div>
          </article>
        ))}
      </div>
